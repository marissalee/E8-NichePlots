...
#' @examples
data(wine)
rownames(wine)<-paste('sample',rownames(wine), sep='_')

tmp<-seq(from=1,to=dim(wine)[1])
groupIndx<-data.frame(rowName=rownames(wine), tmp)
...
groups = tmp; ellipse = FALSE; ellipse.prob = 0.68
...
ggbiplot_ptsize <- function(pcobj, choices = 1:2, scale = 1, pc.biplot = TRUE, 
                     obs.scale = 1 - scale, var.scale = scale, 
                     ...
                     varname.abbrev = FALSE, ...)
{
  ...
  if(!is.null(df.u$labels)) {
    ...
    if(!is.null(df.u$groups)) {
      g <- g + geom_point(aes(color = groups, size=groups), alpha = alpha)
    } else {
