...
#Fxn to run the models with reference conditions PCs (excluding linked response variable) and Microstegium biomass predicting different y variables
ScaleFit_q2<-function(diffVar.basic, df.mod, scale_xs=FALSE, modelFormulaCode, externalPercBA_AM){
  ...
  if(diffVar.basic %in% c('noi','ammonifd')){
    df.mod.used$PC2.use<-df.mod.used$PC2 
    #want to force PC2 to represent increases in forest density (more trees, less light)
  }
  if(diffVar.basic %in% c('nitrifd','minzd')){
    df.mod.used$PC2.use<-df.mod.used$PC2 *-1 #because the variable loadings PC2 are flipped for these ordinations
    #want to force PC2 to represent increases in forest density (more trees, less light)
  }
  ...
  if(modelFormulaCode == 'PC1*PC2*Mv'){
    #fit model (0-5cm, T)
    modT<-lmer(select.diffVarT ~ PC1 * PC2.use * mv_g.m2_logt + (1|year), data=df.mod.used)
    summaryModT<-summary(modT)
    #fit model (5-15cm, B)
    modB<-lmer(select.diffVarB ~ PC1 * PC2.use * mv_g.m2_logt + (1|year), data=df.mod.used)
  ...
  if(modelFormulaCode == 'PC1*PC2*AM*Mv'){
    #fit model (0-5cm, T)
    modT<-lmer(select.diffVarT ~ PC1 * PC2.use * MycBin * mv_g.m2_logt + (1|year), data=df.mod.used)
    summaryModT<-summary(modT)
    #fit model (5-15cm, B)
    modB<-lmer(select.diffVarB ~ PC1 * PC2.use * MycBin * mv_g.m2_logt + (1|year), data=df.mod.used)
    summaryModB<-summary(modB)
  }
